                                              ...
                                              unsigned int likelihoodSkip)
{
  ...
  m_matcher.setMatchingParameters(m_refMap_ptr, urange, range, sigma, kernsize,
                                  lopt, aopt, iterations, likelihoodSigma,
                                  likelihoodSkip);
                                                 ...
                                                 double str, double stt)
{
  ...
  ROS_INFO_STREAM(
      " -srr " << srr << " -srt " << srt << " -str " << str << " -stt " << stt);
}
...
void GridSlamProcessor::init(unsigned int size, const Point& center, int xmax,
                             int ymax, double delta,
                             const OrientedPoint& initialPose)
{
  m_delta = delta;
  ...
  m_refMap_ptr = make_shared<ScanMatcherMap>(center, xmax * delta, ymax * delta,
                                             delta);
          ...
          && ((reading.getTime() - last_update_time_) > period_)))
  {
    ...
    if (m_count > 0)
    {
      ...
      for (auto& it : m_particles)
      {
        ...
        ROS_DEBUG_STREAM("pose: " << i << " x: " << it->pose.x << " y: " << it->pose.y);
      }
