...
public:
    ...
    /**This class defines the the node of reversed tree in which the
       trajectories are stored.
       Each node of a tree has a pointer to its parent and a counter indicating
       the number of childs of a node.
       The tree is updated in a way consistent with the operation performed on
       the particles.
     */
//    struct TNode
//    {
...
//
//      /**Destroys a tree node, and consistently updates the tree. If a node
//         whose parent has only one child is deleted,
...
//      TNode *node;
    ...
    void setMotionModelParameters(double srr,
                                  double srt,
                                  double str,
                                  double stt);
    void setUpdateDistances(double linear,
                            double angular,
...
private:
    ...
    inline bool resample(const double       *plainReading,
                         int                 adaptParticles,
...
//  typedef std::multimap < const GridSlamProcessor::TNode *, GridSlamProcessor::TNode * > TNodeMultimap;
  ...
  //  double sumScore = 0;
  //
  //  for (ParticleVector::iterator it = m_particles.begin(); it !=
  // m_particles.end(); it++)
  //  {
  ...
  //      if (m_infoStream)
  //      {
  //        m_infoStream << "Scan Matching Failed, using odometry. Likelihood="
  // << l << std::endl;
  //        m_infoStream << "lp:" << m_lastPartPose.x << " " << m_lastPartPose.y
  // << " " << m_lastPartPose.theta
  //            << std::endl;
  ...
  //
  //  if (m_infoStream)
  //    m_infoStream << "Average Scan Matching Score=" << sumScore /
  // m_particles.size() << std::endl;
  // }
  //
  ...
  //  m_neff = 0;
  //
  //  for (std::vector<Particle>::iterator it = m_particles.begin(); it !=
  // m_particles.end(); it++)
  //  {
  ...
  //  m_neff = 0;
  //
  //  for (std::vector<double>::iterator it = m_weights.begin(); it !=
  // m_weights.end(); it++)
  //  {
  //    *it = *it / wcum;
  //    double w = *it;
  ...
  // adaptSize, const RangeReading *reading)
  // {
  ...
  //      m_matcher.registerSeenCells(it->seenCells, *m_refMap_ptr, it->pose,
  // plainReading);
...
//    std::cerr << "here it ends" << std::endl;
