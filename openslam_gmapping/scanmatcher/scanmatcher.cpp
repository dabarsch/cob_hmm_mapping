...
void ScanMatcher::computeActiveArea(ScanMatcherMap & map, const OrientedPoint& p, const double *readings)
{
  ...
  {
    min.x = lp.x;
    std::cerr << "Laser off the map" << std::endl;
  }
  ...
  {
    if ((*r > m_laserMaxRange) || (*r == 0.0) || std::isnan(*r))
      continue;
    ...
    {
      min.x = phit.x;
      std::cerr << "Beam off the Map" << std::endl;
    }
  ...
  {
    std::cerr << "Compute Active Area tries to resize the Map" << std::endl;
    ...
    // << lmax.y << endl;
    // cerr << "BOUNDARY OVERRIDE " << min.x << " " << min.y << " " << max.x <<
    // " " << max.y << endl;
  ...
  for (const double *r = readings + m_initialBeamsSkip; r < readings + m_laserBeams; r++, angle++)
    if (m_generateMap)
    {
      ...
      if (d < m_usableRange)
      {
        ...
        map.cell(p1).updateNew(hit, phit);
 ...
 */
void ScanMatcher::registerActiveCells(SmUnorderedMap& activeCells, const ScanMatcherMap& map, const OrientedPoint & p,
                                      const double *readings)
{
  OrientedPoint lp = p;
  ...
  for (const double *r = readings + m_initialBeamsSkip; r < readings + m_laserBeams; r++, angle++)
    if (m_generateMap)
    {
      ...
      for (int i = 0; i < line.num_points - 1; i++)
      {
        ...
        // HMM Extensifdson
        if (activeCells.count(line.points[i]) == 0)
        {
        ...
        {
          assert(activeCells.count(line.points[i]) == 1);
          activeCells.at(line.points[i])->updateNew(miss, Point(0, 0));
        }
...
void ScanMatcher::registerSeenCells(PointUnoSet & seenCells, const ScanMatcherMap& map, const OrientedPoint & p,
                                    const double *readings)
{
  OrientedPoint lp = p;
  ...
  for (double xx = -m_llsamplerange; xx <= m_llsamplerange; xx += m_llsamplestep)
    for (double yy = -m_llsamplerange; yy <= m_llsamplerange; yy += m_llsamplestep)
      for (double tt = -m_lasamplerange; tt <= m_lasamplerange; tt += m_lasamplestep)
      {
        ...
        assert(!std::isnan(sm.likelihood));
        moveList.push_back(sm);
      }
